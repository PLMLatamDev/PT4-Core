...
<div class="col-md-12" style="padding-left: 10px; padding-right: 0px;">
	<div class="box-body" id="prescriptionGrid">
		...
		<c:forEach var="substance" items="${substancesResult}" varStatus="loop">
			<c:if test="${loop.index%2==0}">
				<div class="renglon gris1">
					<div class="tab-1">
						<a href="${pageContext.request.contextPath}/busqueda/medicamentos-sustancia/<c:out value="${substance.getActiveSubstanceId()}"/>">${substance.getDescription()}</a>
					</div>
				</div>
			</c:if>
			<c:if test="${loop.index%2!=0}">
